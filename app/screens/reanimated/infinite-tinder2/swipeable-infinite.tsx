...
export const SwipeableInfinite = observer(
  function SwipeableInfinite(props: SwipeableInfiniteProps, ref: Ref<SwipeInfiniteHandler>) {
    const { profile, onSwipe, onTop, index } = props
    ...
    const onTopValue = useDerivedValue(() => {
      return onTop === true ? withTiming(1) : withTiming(0)
    }, [onTop])

    // const bottomValue = useDerivedValue(() => {
    //   return onTop === true ? withTiming(5) : withTiming(0)
    // }, [onTop])

    const MAX_INDEX = 3

    // console.log(extraRightOffset.value)

    const rOVERLAY = useAnimatedStyle(() => {
      const scale = interpolate(onTopValue.value, [0, 1], [0.7 + 0.1 * index, 1])
      // const opacity = interpolate(scaleValue.value, [])

      return {
        // scale,
        // opacity,
        // bottom: 10 * index + extraRightOffset.value,
        // bottom: 30 * index + bottomValue.value,
        bottom: 30 * index,
        // transform: [{ scale: 0.7 + 0.1 * index }],
        transform: [{ scale }],
      }
    })
